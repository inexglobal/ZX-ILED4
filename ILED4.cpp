...
//  CONSTRUCTOR
//
ILED4::ILED4(const uint8_t address, TwoWire *wire)
{
  _address = address;
  _wire = wire;
}
...
void ILED4::setBlink(uint8_t value)
{
...
uint8_t ILED4::getBlink()
{
  return _blink;
}


void ILED4::setBrightness(uint8_t value)
{
...
uint8_t ILED4::getBrightness()
{
  return _bright;
}


void ILED4::setDigits(uint8_t value)
{
  _digits = value > 4 ? 4 : value;
}


uint8_t ILED4::getDigits()
{
  return _digits;
}


void ILED4::suppressLeadingZeroPlaces(uint8_t value)
{
...
//  0000..FFFF
bool ILED4::displayHex(uint16_t n)
{
  uint8_t x[4], h, l;
  h = n >> 8;
  l = n & 0xFF;
...
//  00.00 .. 99.99
bool ILED4::displayDate(uint8_t left, uint8_t right, bool lz)
{
...
//  00:00 .. 99:99
bool ILED4::displayTime(uint8_t left, uint8_t right, bool colon, bool lz)
{
  bool inRange = ((left < 100) && (right < 100));
  uint8_t x[4];
  x[0] = left / 10;
...
bool ILED4::displaySeconds(uint16_t seconds, bool colon, bool lz)
{
...
bool ILED4::displayFloat(float f, uint8_t decimals)
{
...
bool ILED4::displayUnit(float f, uint8_t decimals, uint8_t unitChar)
{
...
//  EXPERIMENTAL
//
bool ILED4::displayFixedPoint0(float f)
{
  bool inRange = ((-999.5 < f) && (f < 9999.5));
  displayFloat(f, 0);
  return inRange;
...
bool ILED4::displayFixedPoint1(float f)
{
...
bool ILED4::displayFixedPoint2(float f)
{
...
bool ILED4::displayFixedPoint3(float f)
{
...
void ILED4::displayTest(uint8_t del)
{
  for (int i = 0; i < 256; i++)
  {
    writePos(0, i);
...
void ILED4::displayRaw(uint8_t *array, bool colon)
{
  writePos(0, array[0]);
  writePos(1, array[1]);
  writePos(2, array[2]);
  writePos(3, array[3]);
  writePos(4, colon ? 255 : 0);
}


bool ILED4::displayVULeft(uint8_t value)
{
...
bool ILED4::displayVURight(uint8_t value)
{
...
void ILED4::display(uint8_t *array)
{
  ...
  writePos(2, charmap[array[2]]);
  writePos(3, charmap[array[3]]);
...
void ILED4::display(uint8_t *array, uint8_t point)
{
  //  debug to Serial
  ...
  writePos(2, charmap[array[2]], point == 2);
  writePos(3, charmap[array[3]], point == 3);
}


void ILED4::displayColon(uint8_t on)
{
  writePos(4, on ? 255 : 0);
}


void ILED4::displayExtraLeds(uint8_t value)
{
  if (value > 30)
  {
    return;  //  no leds.
...
void ILED4::dumpSerial(uint8_t *array, uint8_t point)
{
...
void ILED4::writeCmd(uint8_t cmd)
{
  _wire->beginTransmission(_address);
  _wire->write(cmd);
  _wire->endTransmission();
...
void ILED4::writePos(uint8_t pos, uint8_t mask)
{
...
void ILED4::writePos(uint8_t pos, uint8_t mask, bool point)
{
  if (point) mask |= 0x80;
  else mask &= 0x7F;
  writePos(pos, mask);
